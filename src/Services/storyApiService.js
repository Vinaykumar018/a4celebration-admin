...
const tokken = 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6IlNoaXZhbnNodSIsImlhdCI6MTczMjE2NTMzOX0.YDu6P4alpQB5QL-74z1jO4LGfEwZA_n_Y29o512FrM8';
const getAuthHeader = () => {
...
export const fetchStoryCategories = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/get-category`, getAuthHeader());
...
export const createStoryCategory = async (formData) => {
  try {
    ...
    const response = await axios.post(`${API_BASE_URL}/create-category`, formData, config);
    return response.data;
  } catch (error) {
...
export const updateStoryCategory = async (id, formData) => {
  try {
    ...
    const response = await axios.put(`${API_BASE_URL}/update-category/${id}`, formData, config);
    return response.data;
  } catch (error) {
...
export const updateStory = async (id, formData) => {
  try {
    ...
    if (!(formData instanceof FormData) && formData._parts) {
      ...
      const hasFile = formData._parts.some(([key, value]) => value instanceof File);
      if (!hasFile) {
        payload = {};
    ...
    const response = await axios.put(`${API_BASE_URL}/update-story/${id}`, payload, config);
    return response.data;
  } catch (error) {
...
export const deleteStoryCategory = async (id) => {
  try {
    ...
    const response = await axios.delete(`${API_BASE_URL}/delete-category/${id}`, getAuthHeader());
    return response.data;
  } catch (error) {
    console.error('Error deleting story category:', error.response?.data || error.message);
    throw error;
  }
};
...
export const fetchActiveStoryCategories = async () => {
  ...
  export const createStory = async (formData) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/add-story`, formData, config);
      return response.data;
    } catch (error) {
  ...
  export const createSubstory = async (formData) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/add-substory`, formData, config);
      return response.data;
    } catch (error) {
  ...
  export const fetchSubstoriesByStoryId = async (storyId) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/${storyId}/substory`, config);
      return response.data;
    } catch (error) {
  ...
  export const fetchSubstoryById = async (storyId, substoryId) => {
    try {
      const config = getAuthHeader();
      const response = await axios.get(`${API_BASE_URL}/${storyId}/substory/${substoryId}`, config);
      return response.data;
    } catch (error) {
  ...
  export const updateSubstory = async (storyId, substoryId, formData) => {
    try {
      ...
      console.error('Error updating substory:', error.response?.data || error.message);
      throw error;
    }
  };