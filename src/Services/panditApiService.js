...
const fetchPandits = {
  ...
  getCategories: async (panditId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/get-category/${panditId}`,
        {
          headers: {
            Authorization: token,
          },
  ...
  delete: async (id) => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || 'Failed to delete pandit',
      );
    }
  },
...
const fetchPoojas = {
  getAll: async () => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || 'Failed to fetch poojas',
      );
    }
  },
};
